...
Common::setTitle('安装插件');
if (!$request->has('upload'))
    Common::setDescription('<a href="add-plugin.php?upload" class="btn btn-sm btn-default">上传</a>');
...
if ($request->has('upload')):
    ...
    <div class="row">
        <div class="col-sm-12">
            <p>请点击下面的区域或将文件拖入该区域来上传插件</p>
            <p>安装完成后，将自动跳转到插件列表页面</p>
            <div id="upload">
    ...
    <input type="hidden" id="upload-url" value="do.php?a=Admin/Plugin/Upload">
<?php else: ?>
    <div class="row">
        <div class="col-sm-12">
            <p>暂不支持在线安装</p>
...
var upload_success = function () {
    window.location.href = "{$options->siteUrl}{$admin_url}plugin.php"
}
JS
);
include "footer.php";
