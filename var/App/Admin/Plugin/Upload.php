...
class Upload extends \App\NoRender
{
    ...
    public function execute(): bool
    {
        ...
        if ($file->isValid()) {
            ...
            if (count($dirs) > 1 || !is_dir($dirs[0]) || // 插件安装要判断是否有Plugin.php
                !file_exists($dir . DIRECTORY_SEPARATOR . 'Plugin.php')) {
                json(['success' => false, 'msg' => '目录结构不正确，安装中止']);
            } else {
                if (file_exists($dist = __ROOT_DIR__ . __PLUGIN_DIR__ . DIRECTORY_SEPARATOR . basename($dir))) {
